...
def get_weather_score(hour, userprefs=default_userprefs):
    ...
    if userprefs["units"] == "f":
        ...
        if hour["feelslike_f"] > userprefs["ideal_temp"] + 10:
            multiplier = 2
        score -= abs(multiplier *
                     (userprefs["ideal_temp"] - hour["feelslike_f"]))
    # Celsius
    else:
        # Adjust multiplier for Celsius to keep it consistent with Fahrenheit results
        multiplier = 1.8
        ...
        if hour["feelslike_c"] > userprefs["ideal_temp"] + 5.55:
            multiplier = 3.6
        score -= abs(multiplier *
                     (userprefs["ideal_temp"] - hour["feelslike_c"]))
...
def get_best_walk(prefs):
    ...
    for hour in conditions["hour"]:
        ...
        logging.debug(
            f"{hour['time']} – {hour['feelslike_f']}°F – Wind: {hour['wind_mph']} MPH – Rain: {hour['will_it_rain']} - Chance of Rain: {hour['chance_of_rain']} - Score: {hour['weather_score']}")
